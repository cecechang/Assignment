...
window.onload = function startScreen(){ //I use onload because I want the user to see the difference on the website after their input
    let userName = sessionStorage.getItem("userName");
    if (!userName || userName.trim() === "") { 
        // If userName is not stored or empty, prompt the user to enter their name
        alert("Welcome to the bomb zone! The goal is to reach 10 points without clicking on any bombs, each move you make will be worth 1 point. Good luck!")
        userName = prompt("To start the game, what's your name?");
        if (userName === null || userName === "") { //error handling in user input, so it won't show 'null'
    ...
    document.querySelectorAll(".game-title").forEach(title => title.textContent = `${userName}'s gameboard`); 
...
for(let i=1; i<=100; i++){ //Using for loop to create 100 <div> elements to reduce repetition
    const cell = document.createElement("div");
    cell.classList.add("cell");

    cell.addEventListener("click", function(){ //event listener is attached to each div element to handle the click event
...
function endGame(isVictory){//the isVictory parameter is a boolean value indicating whether the game ended in victory (true) or defeat (false)
    if (isVictory){
...
function revealAllBombs() {
    ...
    for (let i = 1; i <= cells.length; i++) {
      ...
      if (bombsList.includes(i)) { //check to see if the current cell index (i) is included in the bombsList; 
...
document.querySelector("footer p").textContent = "Copyright © " + currentYear;